...
脚本新增了维护时段检测功能，用于在预定的维护时间（比如夜间或校园网络例行维护时段）暂停自动登录与频繁的网络请求，避免不必要的重试。主要行为与配置如下：

- 新增方法 `is_maintenance_time()`：检查当前时间是否处于配置的维护时段（按星期几与时间段判断）。
- 新增方法 `wait_until_maintenance_end()`：如果处于维护时段，脚本会进入等待循环，在维护结束前暂停登录尝试，并定期记录维护剩余时间以便观察。
- 在主循环中集成维护检查：每次循环开始时会先判断是否处于维护时段，若是则进入等待直到维护结束后再继续正常检测/登录流程。

维护配置已加入到 `config.json` 的 `maintenance` 段，字段说明：

- enabled: 是否启用维护时段检测（true/false）。
- weekdays: 维护日，使用数字表示 0=周一，6=周日。示例：`[0,1,2,3,4]` 表示周一到周五。
- start_time: 维护开始时间，格式为 `"HH:MM"`，例如 `"00:00"`。
- end_time: 维护结束时间，格式为 `"HH:MM"`，例如 `"06:00"`。
- maintenance_check_interval: 在维护时段内的检查间隔（秒），脚本会以此间隔轮询何时结束维护。

示例（片段，完整配置在脚本首次运行时会生成）：
...
默认行为说明：脚本默认在周一至周五的 00:00–06:00 期间视为维护时段并暂停自动登录（当 `maintenance.enabled` 为 `true` 时）。维护期间脚本会在日志中输出剩余维护时间，并每隔 `maintenance_check_interval` 秒重新检查是否结束，结束后自动恢复常规工作流程。
